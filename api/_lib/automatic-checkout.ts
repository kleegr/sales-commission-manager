...
export async function reconcileCheckout(db:Database,tenantId:string,eventId:string,reader= gatewayPage){
 ...
 return db.transaction(async sql=>{
  ...
  for(const p of succeeded){
   ...
   const saved=await recordPayment(sql,u,{clientId:lead.id,eventKey:key,externalId:p._id,source:'ghl',date,amountMinor:amount,currency,status:'confirmed',productId,notes:'Automatically verified Kleeger checkout'},{campaignId:c.campaign_id,referrerId:c.salesperson_id});
   ...
   if(minor(refunded)>minor(previous))await recordRefund(sql,u,{paymentId:saved.id,eventKey:`refund-total:${p._id}:${refunded}`,amountMinor:(minor(refunded)-minor(previous)).toString(),date:dateInZone(p.updatedAt||p.createdAt,w.timezone),reason:'Verified cumulative Kleeger refund'});
