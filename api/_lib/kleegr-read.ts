...
export async function gatewayPage(locationId:string,resource:string,offset=0,fetchImpl:typeof fetch=fetch,params:Record<string,string>={}){
 ...
 if(!r.ok){
  ...
  if(['appointment_scope_mismatch','contact_scope_mismatch','page_scope_mismatch'].includes(failure.error))throw new DirectoryError('gateway_scope_mismatch','Kleeger returned a record that could not be verified against this campaign and workspace. No referral was credited.',502);
  ...
  if(failure.error==='scope_required')throw new DirectoryError('scope_required',`The connected Kleeger app needs permission to read ${resource}. Reconnect it in Smart Productivity after enabling the required access.`,502);
