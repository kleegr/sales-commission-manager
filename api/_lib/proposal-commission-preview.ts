...
export async function proposalCommissionPreview(db:SQL,row:any){
  ...
  let charge=clientId?Number((await db.query("SELECT count(*)::int n FROM payments WHERE tenant_id=$1 AND client_id=$2 AND receipt_status='confirmed' AND parent_payment_id IS NULL",[tenant,clientId])).rows[0].n)+1:1;
