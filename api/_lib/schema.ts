...
//
// This is the canonical, multi-tenant relational schema for the Commission
// Manager. It is imported by:
//   - api/_lib/repository.ts  (runtime ensureSchema on Vercel serverless)
//   - scripts/migrate.ts      (CLI migrations against Neon)
// and is mirrored, for humans, in migrations/0001_init.sql.
//
// DESIGN NOTES
//  - Multi-tenant from day one. Every business row carries `tenant_id` and is
//    scoped by it in all queries. A "tenant" == one GoHighLevel location /
//    sub-account. Tenants roll up to an optional agency_account.
//  - GoHighLevel-ready: tenants.ghl_location_id, clients.ghl_contact_id /
//    ghl_opportunity_id, payments.source / external_payment_id, and dedicated
//    ghl_connections + integration_events tables exist now (nullable / unused)
//    so the OAuth install + webhook phase can be added without a migration of
//    existing data.
//  - TEXT primary keys: we reuse the application's own string IDs (e.g.
//    "sp_jordan", "cm_…") so a full AppData snapshot round-trips losslessly
//    with no ID remapping.
//  - Date-like fields are TEXT holding ISO `yyyy-mm-dd` (or full ISO for
//    payout timestamps) to round-trip exactly with the app + commission engine,
//    which do date math on those strings. Pure-bookkeeping tables use real
//    timestamptz.
//  - commission_rules keeps both typed columns (for SQL reporting) AND a
//    `metadata` JSONB copy of the original rule, so the polymorphic rule union
//    rebuilds with perfect fidelity.
//
// All statements are idempotent (IF NOT EXISTS), so ensureSchema() is safe to
...
/** Every table in the schema, child-first, for safe per-tenant DELETE. */
