...
import {listProductLinks} from './product-links.js';
...
const resources:Record<string,ResourceSpec>={
  ...
  logins:{table:'users',columns:'r.id,r.name,r.email,r.role,r.status,r.salesperson_id,r.kleegr_user_id,r.role_managed_locally',search:"r.name||' '||r.email",admin:true},
  ...
  links:{table:'campaign_participants',columns:`r.*,(SELECT sp.name FROM salespeople sp WHERE sp.tenant_id=r.tenant_id AND sp.id=r.salesperson_id) AS salesperson_name,(SELECT jsonb_build_object('name',c.name,'destination',c.destination_url,'mode',c.tracking_policy->>'automation','status',c.status) FROM campaigns c WHERE c.tenant_id=r.tenant_id AND c.id=r.campaign_id) AS campaign_context`,search:'r.link_id',scope:'r.salesperson_id'},
...
export async function listResource(db:SQL,u:SessionUser,resource:string,f:any={}){
  ...
  // Tracking links: admin sees all; a salesperson sees only their own links.
  if(resource==='productLinks')return listProductLinks(db,u,isAdmin(u)?f:{...f,salespersonId:u.salespersonId||'__none__'});
  ...
  const checkoutInstalled=resource==='campaigns'&&(await db.query("SELECT to_regclass('tracker_inbox') IS NOT NULL AS ready")).rows[0].ready;
  if(resource==='campaigns')for(const campaign of rows){
    ...
    campaign.test_orders=checkoutInstalled?(await db.query("SELECT count(*)::int AS n FROM tracker_inbox e JOIN referral_clicks k ON k.tenant_id=e.tenant_id AND k.id=e.payload->>'clickId' WHERE e.tenant_id=$1 AND k.campaign_id=$2 AND e.provider='ghl-checkout' AND e.status='test_calculated' AND ($3::text[] IS NULL OR k.salesperson_id=ANY($3::text[]))",[u.tenantId,campaign.id,ids])).rows[0].n:0;
  }
