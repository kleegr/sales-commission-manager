...
import {saveSalesman,importSalesmen,createStructure,updateStructure,mediaFolder,saveMediaFile,readFile,experienceRead,preferences} from './tracker-experience.js';
import {parseSalesmanCSV} from '../../src/lib/salesman-csv.js';
...
try{
  ...
  await check('G: draft reservations appear in balances and cannot enter a second draft',async()=>{await assert.rejects(tx(c=>createPayout(c,u,{salespersonId:alice,entryIds:[earning.id]})),{code:'entries_unavailable'});const balances=await report(db,u,{});assert.equal(balances.earnings.find((r:any)=>r.currency==='USD').reserved_minor,'10000');assert.equal((await listResource(db,u,'ledger',{eligible:'1',salespersonId:alice})).rows.some((r:any)=>r.id===earning.id),false);});
  ...
  await check('held future earnings cannot be reserved for payout',async()=>{const p=await tx(c=>recordPayment(c,u,{...receipt,eventKey:'future',date:'2099-01-01'}));const e=(await db.query('SELECT id FROM commission_ledger WHERE payment_id=$1',[p.id])).rows[0];await assert.rejects(tx(c=>createPayout(c,u,{salespersonId:alice,entryIds:[e.id]})),{code:'entries_unavailable'});const eligible=await listResource(db,u,'ledger',{eligible:'1',salespersonId:alice,currency:'USD',limit:'100'});assert.equal(eligible.rows.some((r:any)=>r.id===e.id||r.id===earning.id),false);assert.ok(eligible.rows.some((r:any)=>BigInt(r.amount_minor)<0n));assert.equal((await listResource(db,{...u,tenantId:'b'},'ledger',{eligible:'1',salespersonId:alice})).total,0);});
  ...
  await check('experience migration is additive and repeatable',async()=>{const before=(await listResource(db,u,'ledger')).total;await pg.exec(EXPERIENCE_SCHEMA_SQL);await pg.exec(EXPERIENCE_SCHEMA_SQL);assert.equal((await listResource(db,u,'ledger')).total,before);});
  await check('manual salesman saves profile without creating login; duplicates and unsafe URLs rejected',async()=>{const before=(await db.query('SELECT count(*)::int n FROM users')).rows[0].n;const sp=await tx(c=>saveSalesman(c,u,{firstName:'New',lastName:'Seller',email:'new@example.test',company:'Acme',website:'https://example.com'}));assert.equal((await experienceRead(db,u,'salesmen',{id:sp.id})).rows[0].tracker_profile.company,'Acme');assert.equal((await db.query('SELECT count(*)::int n FROM users')).rows[0].n,before);await assert.rejects(tx(c=>saveSalesman(c,u,{firstName:'New',lastName:'Seller',email:'NEW@example.test'})),{code:'duplicate_email'});await assert.rejects(tx(c=>saveSalesman(c,u,{id:sp.id,firstName:'New',lastName:'Seller',email:'bob@example.test'})),{code:'duplicate_email'});await assert.rejects(tx(c=>saveSalesman(c,u,{firstName:'Unsafe',lastName:'Seller',email:'unsafe@example.test',website:'javascript:alert(1)'})));});
  await check('CSV handles quoted commas, embedded newlines and rejects unclosed fields',()=>{assert.equal(parseSalesmanCSV('First name,Last name,Email\r\n"A, B",Seller,a@example.test')[1][0],'A, B');assert.equal(parseSalesmanCSV('First,Last\n"A\nB",C')[1][0],'A\nB');assert.throws(()=>parseSalesmanCSV('First,Last\n"A,B'));});
  await check('salesman import previews, skips duplicates and rolls back all invalid rows',async()=>{const rows=[{firstName:'CSV',lastName:'Seller',email:'csv@example.test'},{firstName:'Duplicate',lastName:'Seller',email:'new@example.test'}];let r=await tx(c=>importSalesmen(c,u,{rows,preview:true}));assert.equal(r.ready,1);assert.equal(r.skipped,1);assert.equal((await db.query("SELECT id FROM salespeople WHERE email='csv@example.test'")).rows.length,0);await assert.rejects(tx(c=>importSalesmen(c,u,{rows:[...rows,{firstName:'Bad',lastName:'Email',email:'invalid'}]})),{code:'invalid_import'});assert.equal((await db.query("SELECT id FROM salespeople WHERE email='csv@example.test'")).rows.length,0);r=await tx(c=>importSalesmen(c,u,{rows}));assert.equal(r.added,1);assert.equal((await tx(c=>importSalesmen(c,u,{rows}))).added,0);await assert.rejects(tx(c=>importSalesmen(c,self,{rows})),{code:'forbidden'});});
  await check('contact import stays in workspace, preserves clients and stable contact identity',async()=>{const foreign=(await tx(c=>createLead(c,{...u,tenantId:'b'},{name:'Foreign Contact',email:'foreign@example.test',source:'manual',date:'2026-01-01'}))).id;await assert.rejects(tx(c=>importSalesmen(c,u,{contactIds:[foreign]})),{code:'invalid_contact'});const original=(await db.query('SELECT * FROM clients WHERE id=$1',[lead])).rows[0];await tx(c=>importSalesmen(c,u,{contactIds:[lead]}));const after=(await db.query('SELECT * FROM clients WHERE id=$1',[lead])).rows[0];assert.deepEqual(after,original);assert.equal((await tx(c=>importSalesmen(c,u,{contactIds:[lead],preview:true}))).skipped,1);});
  await check('structure creation is atomic when a selected assignment conflicts',async()=>{const before=(await listResource(db,u,'campaigns')).total,versions=(await listResource(db,u,'plans')).total;await assert.rejects(tx(c=>createStructure(c,u,{name:'Conflicting wizard',effectiveFrom:'2026-07-01',config:plan,participantIds:[alice],status:'active',conversionMode:'native',windowDays:30})),{code:'overlapping_assignment'});assert.equal((await listResource(db,u,'campaigns')).total,before);assert.equal((await listResource(db,u,'plans')).total,versions);});
  await check('structure update edits fields atomically, reconciles salesmen and keeps link ids stable',async()=>{
    const s1=(await tx(c=>saveParticipant(c,u,{name:'Update One',email:'update1@example.test',role:'salesperson',status:'active'}))).id;
    const s2=(await tx(c=>saveParticipant(c,u,{name:'Update Two',email:'update2@example.test',role:'salesperson',status:'active'}))).id;
    const created=await tx(c=>createStructure(c,u,{name:'Editable campaign',description:'Before',effectiveFrom:'2026-01-01',config:plan,participantIds:[s1],status:'active',conversionMode:'external',destinationUrl:'https://example.com/offer',windowDays:30}));
    const link=(await db.query('SELECT link_id FROM campaign_participants WHERE campaign_id=$1 AND salesperson_id=$2',[created.id,s1])).rows[0].link_id;
    const edited={id:created.id,name:'Edited campaign',description:'After',effectiveFrom:'2026-02-01',config:{...plan,rules:[{...plan.rules[0],value:'2000'}]},participantIds:[s2],status:'paused',conversionMode:'external',destinationUrl:'https://example.com/offer2',windowDays:45,touch:'last',terms:'New terms'};
    const updated=await tx(c=>updateStructure(c,u,edited));
    const row=(await db.query('SELECT * FROM campaigns WHERE id=$1',[created.id])).rows[0];
    assert.equal(row.name,'Edited campaign');assert.equal(row.description,'After');assert.equal(row.status,'paused');assert.equal(row.destination_url,'https://example.com/offer2');assert.equal(row.tracking_policy.windowDays,45);assert.equal(row.tracking_policy.touch,'last');assert.equal(row.terms,'New terms');assert.equal(row.plan_version_id,updated.versionId);assert.notEqual(updated.versionId,created.versionId);
    const removed=(await db.query('SELECT * FROM campaign_participants WHERE campaign_id=$1 AND salesperson_id=$2',[created.id,s1])).rows[0];
    const added=(await db.query('SELECT * FROM campaign_participants WHERE campaign_id=$1 AND salesperson_id=$2',[created.id,s2])).rows[0];
    assert.equal(removed.active,false);assert.equal(removed.link_id,link);assert.equal(added.active,true);
    assert.equal((await db.query('SELECT effective_to FROM plan_assignments WHERE campaign_id=$1 AND salesperson_id=$2',[created.id,s1])).rows.every((a:any)=>a.effective_to!==null),true);
    // Re-adding a removed salesman revives the identical shared link id.
    await tx(c=>updateStructure(c,u,{...edited,participantIds:[s1,s2],status:'active'}));
    const revived=(await db.query('SELECT * FROM campaign_participants WHERE campaign_id=$1 AND salesperson_id=$2',[created.id,s1])).rows[0];
    assert.equal(revived.active,true);assert.equal(revived.link_id,link);
    // Unchanged rules keep the same immutable plan version; each salesman keeps exactly one open campaign assignment.
    const again=await tx(c=>updateStructure(c,u,{...edited,participantIds:[s1,s2],status:'active'}));
    assert.equal(again.versionId,updated.versionId);
    assert.equal((await db.query('SELECT count(*)::int AS n FROM plan_assignments WHERE campaign_id=$1 AND plan_version_id=$2 AND effective_to IS NULL',[created.id,updated.versionId])).rows[0].n,2);
    // Invalid rules roll the whole update back, and cross-tenant ids are rejected.
    await assert.rejects(tx(c=>updateStructure(c,u,{...edited,name:'Should not persist',config:{...plan,rules:[]}})));
    assert.equal((await db.query('SELECT name FROM campaigns WHERE id=$1',[created.id])).rows[0].name,'Edited campaign');
    await assert.rejects(tx(c=>updateStructure(c,{...u,tenantId:'b'},edited)),{code:'not_found'});
    await assert.rejects(tx(c=>updateStructure(c,u,{...edited,participantIds:['not-in-tenant']})),{code:'not_found'});
  });
