...
Add `sales.kleeger.com` to this project's Vercel Settings > Domains and create the CNAME record Vercel specifies at the DNS provider for `kleeger.com`. Wait for valid configuration/HTTPS. Then set the Production environment variable `PROPOSAL_PUBLIC_URL=https://sales.kleeger.com` and redeploy. This setting affects copied and emailed approval links only; it does not change GHL checkout/invoice domains. Leave it unset until DNS works. Existing token paths work on any verified domain serving this same project/database. Generating a new link rotates the old token. Business Setup's website field is contact branding, not domain mapping.
