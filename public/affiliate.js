...
(()=>{
 ...
 // Some GHL bundles retain fetch before this script loads and encrypt their saved receipts.
 // Observe JSON decoding for the same exact order endpoints, preserving the original response.
 if(typeof Response!=='undefined'){for(const method of ['json','text']){const original=Response.prototype[method];Response.prototype[method]=function(...args){const url=this.url;return original.apply(this,args).then(body=>{try{const parsed=new URL(url,location.href);if(['services.leadconnectorhq.com','backend.leadconnectorhq.com'].includes(parsed.hostname)&&['/payments/orders','/funnels/order-form/order','/funnels/order-form/checkout'].includes(parsed.pathname))observe(url,method==='text'?JSON.parse(body):body);}catch{}return body;});};}}
