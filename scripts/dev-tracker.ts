...
const server=await preview({...config,configFile:false,preview:{host:'127.0.0.1',port:4173,strictPort:true},plugins:[...(config.plugins||[]),{name:'isolated-api',configurePreviewServer(vite:any){vite.middlewares.use(async(req:any,res:any,next:any)=>{
  const url=new URL(req.url,'http://127.0.0.1:4173');if(!url.pathname.startsWith('/api/'))return next();
...
});}}]});console.log('Isolated browser test server: http://127.0.0.1:4173 — embedded Postgres, fake identities, no live credentials.');
