...
import {EXPERIENCE_SCHEMA_SQL} from '../api/_lib/tracker-experience-schema.js';
...
console.log(JSON.stringify({host:target.hostname,database:target.pathname,migration:'0013_tracker_experience'}));
...
await database.transaction(async db=>{await db.query("SET LOCAL lock_timeout='5s'");await db.query("SET LOCAL statement_timeout='120s'");await db.query("SELECT pg_advisory_xact_lock(hashtext('sales-tracker-migration'))");await db.query(EXPERIENCE_SCHEMA_SQL);});
