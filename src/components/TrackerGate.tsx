...
export function TrackerGate({resource,children}:{resource:string;children?:ReactNode}){const s=useTracker();if(s.loading)return <p role="status">Loading workspace…</p>;if(s.error)return <div role="alert"><p>{s.error}</p><button onClick={s.refresh}>Retry</button></div>;if(!s.installed)return children?<>{children}</>:<p>The reviewed Sales Tracker database update is required to activate this area.</p>;if(resource==='agency')return <TrackerAgency/>;if(resource==='dashboard')return <Dashboard/>;if(resource==='products')return <Products/>;if(resource==='people')return <Salesmen/>;if(resource==='plans')return <Structures/>;if(resource==='payouts')return <Payouts/>;if(resource==='media')return <Media/>;if(resource==='settings')return <Settings/>;return <TrackerPage key={resource} resource={resource}/>;}
