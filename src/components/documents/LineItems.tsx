      ...
      {loading ? (
        ...
        <>
          ...
          {<div aria-label="Matching catalog products" className="proposal-product-catalog">
            {available.length===0?<p className="p-3 text-sm text-slate-500">No products match these filters. Try another search or clear the filters.</p>:available.map(p=><div key={p.id} className={`proposal-catalog-card${selectedIds.has(p.id)?' is-selected':''}${requiredBy.has(p.id)?' is-required':''}`}>
