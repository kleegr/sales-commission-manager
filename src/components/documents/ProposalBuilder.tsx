...
import {productQuote} from '../../lib/proposal-suite';
...
export function ProposalBuilder(p: Props) {
  ...
  function validation(index:number) {
    ...
    if(index===1){
      ...
      for(const item of items){const product=p.products.find(x=>x.id===item.productId);if(!product)return 'A selected product is no longer available.';const quote=productQuote(product,item.qty,items);if(quote.error)return quote.error;if(BigInt(item.unitPriceMinor)<BigInt(quote.floorMinor))return 'A price is below its configured minimum.';}
    }
