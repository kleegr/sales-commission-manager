...
export function Layout({children}:{children:ReactNode}){
 ...
 const primary=[{to:role==='owner'?'/workspace-overview':'/',label:'Dashboard'},{to:'/products',label:'Products'},{to:'/plans',label:p.structureLabel},{to:'/people',label:p.salesmanLabel},{to:'/users',label:'Users'},{to:'/documents',label:'Proposals'},{to:'/notifications',label:'Notifications'},{to:'/payouts',label:p.payoutLabel},{to:'/tracker-settings',label:'Settings'}].filter(i=>allowed(i.to));
