...
export default function CampaignLinks({campaignId,status,destination}:{campaignId:string;status:string;destination?:string}){
 ...
 return <section aria-label="Campaign salesmen">
  ...
  {current&&destination&&(!['test','live','auto'].includes(automation)||!connected)&&<p className="st-help">Funnel setup is needed to track orders. <Action onClick={()=>setAdvanced(true)}>Open setup</Action></p>}
