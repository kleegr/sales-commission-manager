...
export default function CampaignLinks({campaignId,status,destination}:{campaignId:string;status:string;destination?:string}){
 ...
 const result=useRemote('links',{campaignId,page:String(page),limit:'25'},refresh);
 const [refreshing,setRefreshing]=useState(false),[activityLoading,setActivityLoading]=useState(false),setupRef=useRef<HTMLDetailsElement>(null);
 // The 15s background poll stays silent; a clicked Refresh stays busy until campaign, links and activity have all reloaded.
 const lastLinks=useRef<any>(null),linksFresh=useRef(false);
 useEffect(()=>{if(result.data!==lastLinks.current||result.error){lastLinks.current=result.data;linksFresh.current=true;}if(refreshing&&linksFresh.current&&!campaign.loading&&!activityLoading)setRefreshing(false);},[result.data,result.error,campaign.loading,activityLoading,refreshing]);
 const busy=refreshing||campaign.loading||result.loading;
 const startRefresh=()=>{linksFresh.current=false;setRefreshing(true);setRefresh(n=>n+1);};
 const openSetup=()=>{setAdvanced(true);requestAnimationFrame(()=>{const el=setupRef.current;if(!el)return;el.scrollIntoView({behavior:'smooth',block:'start'});el.classList.remove('st-flash');void el.offsetWidth;el.classList.add('st-flash');setTimeout(()=>el.classList.remove('st-flash'),1700);});};
 useEffect(()=>{if(!current||submissionSource)return;const c=new AbortController();let busy=false;const load=async()=>{if(busy||c.signal.aborted)return;busy=true;setActivityLoading(true);try{const r=await fetch(`/api/operations?resource=campaignActivity&includeSalesmen=1&campaignId=${encodeURIComponent(campaignId)}`,{signal:c.signal});const b=await r.json();if(!r.ok)throw Error(b.message||'Salesman results could not be loaded.');if(!c.signal.aborted){setActivity(b);setActivityError('');}}catch(e){if(!c.signal.aborted)setActivityError((e as Error).message);}finally{busy=false;setActivityLoading(false);}};void load();const timer=setInterval(()=>{if(document.visibilityState==='visible')void load();},15000);return()=>{c.abort();clearInterval(timer);};},[campaignId,refresh,!!current,submissionSource]);
 ...
 return <section className="st-links" aria-label="Campaign salesmen">
  <div className="st-links-heading"><div><h3>Salesman performance</h3><p className="st-help">See who brought each order. Share their personal link.</p></div><Action disabled={busy} onClick={startRefresh}>{busy?'Refreshing…':'Refresh'}</Action></div>
  {status!=='active'&&<p className="st-help">This campaign is not active. Activate it before sharing links.</p>}
  {current&&destination&&(!['test','live','auto'].includes(automation)||!connected)&&<div className="st-inline-banner"><span>This source is not yet connected for automatic purchases.</span><Action onClick={openSetup}>Open setup</Action></div>}
  ...
  <details ref={setupRef} className="st-disclosure" style={{marginTop:20}} open={advanced} onToggle={e=>setAdvanced(e.currentTarget.open)}><summary>Links & setup</summary>
