...
console.log("\n[Roles · test-results dashboard target (tracker Payouts banner)]");
// The Payouts "View test results" link must send each role to a dashboard it
// can actually open (the Guard drops query params on redirect, so linking a
// blocked route silently loses ?mode=test). Mirrors the selection logic in
// src/pages/tracker/Payouts.tsx.
const testDash = (r: Role) =>
  canAccess(r, "/") ? "/" : canAccess(r, "/workspace-overview") ? "/workspace-overview" : null;
ok("admin's test dashboard is /", testDash("admin") === "/");
ok("sales manager's test dashboard is /", testDash("sales_manager") === "/");
ok("owner's test dashboard is /workspace-overview (not the blocked /)", testDash("owner") === "/workspace-overview");
ok("salesperson gets no dashboard link (neither route accessible)", testDash("salesperson") === null);
ok("affiliate gets no dashboard link", testDash("affiliate") === null);
ok("partner gets no dashboard link", testDash("partner") === null);
ok("every role's test dashboard, when present, is accessible", ALL_ROLES.every((r) => {
  const d = testDash(r);
  return d === null || canAccess(r, d);
}));
