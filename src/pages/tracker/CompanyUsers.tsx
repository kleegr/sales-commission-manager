...
import {companyUsers,type CompanyIdentity} from '../../lib/company-users';
...
export default function CompanyUsers(){
  ...
  return <div className="st-page company-users">
    ...
    <section className="company-users-panel" aria-label="Company users">
      <div className="company-users-filters"><label>Search users<input placeholder="Search name or email…" value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}}/></label><label>App role<select value={role} onChange={e=>{setRole(e.target.value);setPage(1);}}><option value="">All roles</option>{Object.keys(ROLE_LABEL).map(r=><option key={r} value={r}>{roleLabel(r)}</option>)}<option value="none">No app access</option></select></label><label>Salesman enrollment<select value={enrollment} onChange={e=>{setEnrollment(e.target.value);setPage(1);}}><option value="">Everyone</option><option value="yes">Enrolled</option><option value="no">Not enrolled</option></select></label></div>
      <p className="company-users-help">App roles control access to Sales Tracker. GoHighLevel roles come from your sub-account. A user can be both an administrator and a salesman.</p>
      {loading&&<p role="status">Loading company users…</p>}
      {!loading&&data&&<><div className="st-table-wrap"><table className="st-table"><thead><tr><th>User</th><th>App role</th><th>Salesman status</th><th>GoHighLevel role</th><th>Source</th></tr></thead><tbody>{visible.map(u=><tr key={u.key}>
    ...
    <p className="company-users-help">“No app access” means there is no linked Sales Tracker login. “Not enrolled” means the user has not been added as a salesman. To enroll someone, open <Link to="/people">Salesman → Add → Import from Users</Link>. Viewing this page does not change anyone’s permissions.</p>
  </div>;
}
