...
export default function CompanyUsers(){
  ...
  return <div className="st-page company-users">
    ...
    <section className="company-users-panel" aria-label="Company users">
      ...
      <p className="company-users-help">User roles control access to Kleeger. Kleeger roles come from your sub-account. A user can be both an administrator and a salesman.</p>
      {loading&&<p role="status">Loading company users…</p>}
      {!loading&&data&&<><div className="st-table-wrap"><table className="st-table"><thead><tr><th>User</th><th>User role</th><th>Salesman status</th><th>Kleeger role</th><th>Source</th><th>Actions</th></tr></thead><tbody>{visible.map(u=><tr key={u.key}>
        ...
        <td>{u.directory?<><span>{u.directory.provider_role||'Not supplied'}</span><small className="st-block">{u.directory.active?'Active in Kleeger':'Inactive in Kleeger'}</small></>:'Not connected'}</td>
        <td>{u.directory?'Kleeger':u.login?'Kleeger login':'Manually added salesman'}{u.matchedByEmail&&<small className="st-block">Login matched by unique email</small>}</td>
        <td><div className="st-row-buttons">{(u.login||u.directory)&&<Button variant="secondary" size="sm" disabled={u.login?.id===user?.id||(u.login?.role==='owner'&&user?.role!=='owner')} onClick={()=>setEditing({person:u,enrollOnly:false})}>Change role</Button>}{!u.salesman&&(u.login||u.directory)&&<Button variant="secondary" size="sm" onClick={()=>setEditing({person:u,enrollOnly:true})}>Make salesman</Button>}</div>{u.login?.id===user?.id&&<small className="st-block">Your account</small>}{!u.login&&!u.directory&&<small>Link a Kleeger user from the salesman profile to enable app access.</small>}</td>
      </tr>)}</tbody></table></div>{!visible.length&&<p className="company-users-empty">{users.length?'No users match these filters.':'No users found. Sync your Kleeger sub-account to load its users.'}</p>}
    ...
    <p className="company-users-help">“No app access” means there is no linked Kleeger login. “Not enrolled” means the user has not been added as a salesman. To enroll someone, open <Link to="/people">Salesman → Add → Import from Users</Link>. Use Change role to manage app access. Only owners can assign Owner access. Kleeger roles remain unchanged.</p>
  </div>;
}
