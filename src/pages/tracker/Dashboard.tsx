...
import {Action,Header,Message,useRemote,useLiveRevision,usePreferences} from '../../components/tracker/Experience';
import {AdminGettingStarted} from '../../components/tracker/GettingStarted';
import ProposalOverview from '../../components/tracker/ProposalOverview';
export default function TrackerDashboard(){
 const pref=usePreferences(),{workspace}=useTracker(),[search]=useSearchParams(),{user}=useAuth();
 const canReviewImports=canAccess((user?.role||'salesperson') as Role,'/sync-review');
 const [from,setFrom]=useState(''),[to,setTo]=useState(''),[campaignId,setCampaign]=useState(''),[refresh,setRefresh]=useState(0);
 const [revision,refreshBalances]=useLiveRevision(),campaigns=useRemote('campaigns',{limit:'100'}),overview=useRemote('balances',{},revision);
 ...
 return <div className="st-page st-dashboard-page"><Header title="Dashboard" description="Track proposals, completed sales and commission payouts in one place."/>
 <AdminGettingStarted refreshToken={refresh}/>
 ...
 <Message error={campaigns.error}/>{from&&to&&from>to?<Message error="Choose an end date on or after the start date."/>:<><ProposalOverview refreshToken={refresh} from={from} to={to} campaignId={campaignId}/><CampaignActivity key={pref.values.dashboardMode} refreshToken={refresh} campaignId={campaignId} from={from} to={to} dashboard initialMode={['live','test'].includes(search.get('mode')||'')?search.get('mode')!:pref.values.dashboardMode}/></>}
 ...
 <div className="st-report-links"><Link to="/reports">Detailed financial reports</Link><Link to="/ledger">Commission ledger</Link>{canReviewImports&&<Link to="/sync-review">Review imports</Link>}</div>
 </div>;
}
